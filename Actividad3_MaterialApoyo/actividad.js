...
let imagePath = 'input/tucan.jpg'; // Changed the variable name to 'imagePath'
let handler = new ImageHandler(imagePath);
...
function merge(alphaFirst, alphaSecond) {
    ...
    if (catPixels && dogPixels) {
        let pixels = [];
        for (let i = 0; i < catPixels.length; i++) {
            if (catPixels[i] && dogPixels[i]) {
                let mergedRow = [];
                for (let j = 0; j < catPixels[i].length; j++) {
                    if (catPixels[i][j] && dogPixels[i][j]) {
                        let mergedPixel = [
                            Math.floor(alphaFirst * catPixels[i][j][0] + alphaSecond * dogPixels[i][j][0]),
                            Math.floor(alphaFirst * catPixels[i][j][1] + alphaSecond * dogPixels[i][j][1]),
                            Math.floor(alphaFirst * catPixels[i][j][2] + alphaSecond * dogPixels[i][j][2])
                        ];
